...
public class Ln extends LimitedIterationsFunction {
    ...
    public BigDecimal calculate(
        ...
        BigDecimal expectedPrecision = new BigDecimal("0.1").pow(precision.scale());
        if (Math.abs(X - 1) <= 1) {
            do {
                prevValue = curValue;
            ...
            } while (expectedPrecision.compareTo((prevValue.subtract(curValue)).abs()) < 0 && i < maxIterations);
